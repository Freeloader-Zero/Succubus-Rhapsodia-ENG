...
=end
  ...
  def make_hold_text(skill, active, target)
    if active.is_a?(Game_Actor)
      ...
      case skill.name
      when "インサート"
        text = "#{active.name}は#{target.name}に挿入した！"
      when "アクセプト"
        text = "#{active.name}は#{brk}自分のアソコに、\n#{target.name}のペニスを挿入した！"
      when "オーラルインサート"
        text = "#{active.name}は#{brk}#{target.name}の口に挿入した！"
      when "オーラルアクセプト"
        text = "#{active.name}は#{brk}#{target.name}のペニスを口に咥えた！"
      when "バックインサート"
        text = "#{active.name}は#{brk}#{target.name}のお尻に挿入した！"
      when "バックアクセプト"
        text = "#{active.name}は自分のお尻に、\n#{target.name}のペニスを挿入した！"
      when "エキサイトビュー"
        text = "#{active.name}は#{brk}#{target.name}の頭に馬乗りになった！"
      when "ドロウネクター"
        text = "#{active.name}は#{brk}#{target.name}のアソコに吸い付いた！"
      when "エンブレイス"
        text = "#{active.name}は#{brk}#{target.name}に抱きついた！"
      when "シェルマッチ"
        text = "#{active.name}は#{brk}#{target.name}と脚を絡め合った！"
      when "ディルドインサート"
        text = "#{active.name}は#{brk}#{target.name}に挿入した！"
      when "ディルドインマウス"
        text = "#{active.name}は#{brk}#{target.name}の口に挿入した！"
      when "ディルドインバック"
        text = "#{active.name}は#{brk}#{target.name}のお尻に挿入した！"
      ...
      when "リリース"
        text = "#{active.name}は#{brk}#{target.name}を振りほどいた！"
      when "インタラプト"
        ...
        text = "#{active.name}は、\n#{partner.name}から#{target.name}を引き離した！"
      end
      ...
      when "インサート"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は自ら脚を広げ、\n#{active.name}の挿入を迎え入れた！"
        else
          text = "#{target.name}は#{active.name}に犯された！"
        end
      when "アクセプト"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
      ...
      when "オーラルインサート"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は自ら口を開け、\n#{active.name}のペニスを咥え込んだ！"
        else
          text = "#{target.name}の口に\n#{active.name}のペニスがねじ込まれた！"
        end
      when "オーラルアクセプト"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は自ら腰を突き出し、\n#{active.name}の口淫を迎え入れた！"
        else
          text = "#{target.name}のペニスは、\n#{active.name}の口に咥え込まれてしまった！"
        end
      when "バックインサート"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は自らお尻を上げ、\n#{active.name}のペニスを菊座に受け入れた！"
        else
          text = "#{target.name}の菊座に、\n#{active.name}のペニスが突き入れられた！"
        end
      when "バックアクセプト"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は自ら仰向けになり、\n#{active.name}の挿入を受け入れた！"
        else
          text = "#{target.name}のペニスは、\n#{active.name}の菊座に飲み込まれてしまった！"
        end
      when "エキサイトビュー"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は自ら仰向けになり、\n#{active.name}の騎乗を受け入れた！"
        else
          text = "#{target.name}の顔が、\n#{active.name}のアソコで押しつぶされた！"
        end
      when "インモラルビュー"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は自ら仰向けになり、\n#{active.name}の騎乗を受け入れた！"
        else
          text = "#{target.name}の顔が、\n#{active.name}のお尻で押しつぶされた！"
        end
      when "エンブレイス"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は力を抜き、\n#{active.name}に身体を預けた！"
        else
          text = "#{target.name}は#{active.name}に抱きつかれた！"
        end
      when "エキシビジョン"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は自ら脚を開き、\n#{active.name}に身体を預けた！"
        else
          text = "#{active.name}は#{target.name}を抱え込み、\n周囲に良く見えるように股を大きく開かせた！"
        end
      when "ペリスコープ"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は自ら腰を突き出し、\n#{active.name}の胸にペニスをうずめた！"
        else
          text = "#{target.name}のペニスは、\n#{active.name}の胸の谷間に飲み込まれた！！"
        end
      when "シェルマッチ"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は自ら脚を広げ、\n#{active.name}を迎え入れた！"
        else
          text = "#{target.name}は#{brk}#{active.name}に脚を絡め取られた！"
        end
      when "ドロウネクター"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
          text = "#{target.name}は自ら指でアソコを広げ、\n#{active.name}の口淫を迎え入れた！"
        else
          text = "#{target.name}のアソコに、\n#{active.name}が吸い付いてきた！"
        end
      when "インサートテイル"
        text = "#{target.name}のアソコに、\n#{active.name}の尻尾が突き入れられた！"
      when "マウスインテイル"
        text = "#{target.name}の口に、\n#{active.name}の尻尾がねじ込まれた！"
      when "バックインテイル"
        text = "#{target.name}の菊座に、\n#{active.name}の尻尾が突き入れられた！"
      when "ディルドインサート"
        if $game_switches[89] == true #キャンセルキーで受け入れた場合
  ...
  def make_unhold_text(target)
    ...
    if target.hold.penis.battler != nil
      ...
      case target.hold.penis.type
      when "♀挿入"
        text.push("#{holder.name}はアソコからペニスを抜いた！")
      when "口挿入"
        text.push("#{holder.name}はペニスから口を離した！")
      when "尻挿入"
        text.push("#{holder.name}は菊座からペニスを抜いた！")
      when "パイズリ"
        text.push("#{holder.name}は胸の谷間からペニスを解放した！")
      end
    end
    if target.hold.vagina.battler != nil
      ...
      when "♀挿入"
        ...
        text.push("#{holder.name}は#{item}を引き抜いた！")
      when "顔面騎乗","クンニ"
        text.push("#{holder.name}は#{target.name}を解放した！")
      when "貝合わせ"
        text.push("#{holder.name}は絡ませた脚をほどいた！")
      end
    end
    if target.hold.mouth.battler != nil
      ...
      when "顔面騎乗","尻騎乗","ぱふぱふ"
        text.push("#{holder.name}は#{target.name}を解放した！")
      when "クンニ","キッス"
        text.push("#{holder.name}は#{target.name}から離れた！")
      end
    end
    if target.hold.anal.battler != nil
    ...
    if target.hold.tail.battler != nil
      ...
      case target.hold.tail.type
      when "♀挿入","口挿入","尻挿入"
        text.push("#{holder.name}は尻尾の攻めから逃れた！")
      end
    end
    if target.hold.tentacle.battler != nil
      ...
      case target.hold.tentacle.type
      when "♀挿入","口挿入","尻挿入"
        text.push("#{holder.name}は触手の攻めから逃れた！")
      end
    end
    if target.hold.dildo.battler != nil
      ...
      case target.hold.dildo.type
      when "♀挿入","口挿入","尻挿入"
        text.push("#{holder.name}はディルドの攻めから逃れた！")
      end
    end
    a = ""
