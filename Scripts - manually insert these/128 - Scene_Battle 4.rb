...
#            add_hold(@skill, @active_battler, target)
          ...
          # その他
            ...
            $game_temp.battle_log_text += "レジストに成功しました\w\q" 
          end
        # 失敗時
        else
          @resist_flag = false
          ...
          if $game_switches[89]
            $game_temp.battle_log_text += @active_battler.name + "は思いとどまった！\w\q"
          else
          # 避けるSEを鳴らし、対象のレジストカウントを＋１する
            Audio.se_play("Audio/SE/063-Swing02", 80, 100)
            $game_temp.battle_log_text += target.name + "に抵抗された！\w\q"
      ...
      # 対象がアクターの場合
        ...
        if $game_temp.resistgame_clear == true
          ...
          # 避けるSEを鳴らし、対象のレジストカウントを＋１する
          Audio.se_play("Audio/SE/063-Swing02", 80, 100)
          $game_temp.battle_log_text += target.name + "は抵抗した！\w\q"
          target.resist_count += 1
          if @skill.element_set.include?(6)
            @hold_shake = false
            hold_effect(@skill, @active_battler, target)
...
#            add_hold(@skill, @active_battler, target)
          ...
          # その他
            ...
            $game_temp.battle_log_text += "レジストに失敗しました\w\q" 
          end
        end
      end
      # 画像変更
        ...
        if target.damage != "Miss"
          ...
          if not @command.element_set.include?(17)
            ...
            if @command.element_set.include?(31) or @command.element_set.include?(32)
              ...
              text3 += "\w" + @active_battler.name + "は " + rebound.to_s + " の快感を受けた！\q" if rebound > 0
            ...
            if @command.element_set.include?(198)
              ...
              if vp_drain > 0
                ...
                text3 += "\w" + @active_battler.name + "は精気を " + vp_drain.to_s + " 吸収した！\w\q"
                if target.sp <= 0
            ...
            if @command.element_set.include?(202)
              if target.level > 1
                target.level -= 1
                text3 += "\w" + "#{target.name}はLv.#{target.level.to_s}になってしまった！"
                # 使用者にストレリブルムの強化項目を通す
                level_drain_text = ""
...
#                end
            ...
            if @active_battler.is_a?(Game_Actor) and @active_battler.have_ability?("洞察力")
              ...
              if target.is_a?(Game_Enemy) and target.checking < 1
                target.checking = 1
                text3 += "\w" + @active_battler.name + "は#{brk}#{$game_temp.battle_target_battler[0].name}の事を調べ上げた！\w\q"
              end
            end
            #------------------------------------------------------
          ...
          if @command.element_set.include?(33)
            ...
            for i in SR_Util.checking_states
              if $data_skills[@skill.id].plus_state_set.include?(i) and target.states.include?(i)
                text1 += "しかし" + target.name + "にはこれ以上効果が無い！\w\q"
                bs = 1
                break
              end
            end
...
#            # ミスした場合は避けるSEを鳴らす
#            Audio.se_play("Audio/SE/063-Swing02", 80, 100)
            $game_temp.battle_log_text += "\w" + target.name + "に効果はなかった！\q"
          end
        end
        #======================================================================================
        # ■特殊
...
=end
          ...
          # バトルログを表示
          $game_temp.battle_log_text += b_actor.name + "が戦闘に入った！\w\q"
          # ステータス画面をリフレッシュ
          @status_window.refresh
          #●バトルトーク関連をリセット
          $game_temp.action_num = 0
