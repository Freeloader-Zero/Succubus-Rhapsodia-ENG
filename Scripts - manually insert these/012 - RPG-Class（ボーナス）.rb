  ...
  class Class
    ...
    def bonus
      ...
      when "レッサーサキュバス","サキュバス","サキュバスロード"
        ...
        # 【カリスマ】（サキュバスロード）
        group.push([1500, 1, 73]) if self.name == "サキュバスロード" 
        #【最高の姿】
      ...
      when "インプ","デビル","デーモン"
        # ツーパフ（デビル以降）
        group.push([200, 0, 91]) if self.name == "デビル" or self.name == "デーモン" 
        # トリックレイド
        ...
        #【小悪魔の連携】（インプ Lv30）
        group.push([2000, 1, 81, 30]) if self.name == "インプ" 
        #【小悪魔の統率】（デビル以降 Lv30）
        group.push([2000, 1, 82, 30]) if self.name == "デビル" or self.name == "デーモン" 
        #【吸精】
        group.push([1000, 1, 70]) if self.name == "デビル" or self.name == "デーモン" 
        # 【生命力】
        group.push([400, 1, 214])
        # 【溢れる生命力】（緑）
        group.push([1000, 1, 215]) if self.color == "緑"
        # 【カリスマ】(デーモン)
        group.push([1200, 1, 73]) if self.name == "デーモン" 
        #【最高の姿】
        group.push([5000, 1, 50, 30]) 
      #------------------------------------------------------------------------
      # ■■ウィッチ種
      ...
      when "スライム","ゴールドスライム"
        # 金を採取する
        group.push([500, 3, 3])  if self.name == "ゴールドスライム"
        # ファストレイド
        group.push([700, 0, 103])
        # トリックレイド
        group.push([700, 0, 104])
        # ネリネブルム
        group.push([600, 0, 175]) if self.name == "スライム"
        # ネリネブルム・アルダ
        group.push([2000, 0, 176, 30]) if self.name == "スライム"
        # 祝福の口付け
        group.push([600, 0, 417]) if self.name == "ゴールドスライム"
        # 【平静】
        group.push([450, 1, 108])
        # 【ダウト】
        group.push([600, 1, 226]) if self.name == "スライム"
        # 【熟練】
        group.push([1000, 1, 124])
        # 【生命力】
        group.push([500, 1, 214]) if self.name == "スライム"
        # 【免疫力】
...
#未        group.push([1000, 0, 227])
      ...
      when "ゴブリン","ギャングコマンダー"
        ...
        # チェック
        group.push([300, 0, 126]) if self.name == "ギャングコマンダー"
        # チャーム
        group.push([1000, 0, 200]) if self.name == "ギャングコマンダー"
        # ペイド・チャーム （Lv40）
        group.push([1800, 0, 201, 40]) if self.name == "ギャングコマンダー"
        # エンブレイス
        group.push([800, 0, 17])
        # 【高揚】
