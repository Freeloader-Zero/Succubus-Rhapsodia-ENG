...
module RPG
  class Skill
    def message(skill, type, myself, user)
      ...
      premess = "#{myname}"
      avoid = "But #{myname} ｑuickly dodged out of the ｗay!"
  ...
  #================================================================================================================================#
      ...
      when "Lesser Succubus ","Succubus", "Verｍiena "
        tail = "flexible tail"
      when "Iｍp","Devil ","Deｍon", "Yuganaught"
        tail = "spaded tail"
      when "Werecat ","Wereｗolf ","Taｍaｍo"
        tail = "fluffy tail"
      when "Fulbeua ", "Neijorage ", "Succubus Lord "
        tail = "glossy tail"
      when "Faｍiliar ", "Rejeo ", "Sylphe"
        tail = "delicate tail"
      when "Gargoyle"
        tail = "sｑuarish tail"
      else
  ...
  #------------------------------------------------------------------------#
      when "服を脱がす"
        if target.is_a?(Game_Enemy) #エネミーを脱がせる
          action = premess + ",\n\m atteｍpts to take off #{targetname}'s clothes!"
          #スライム系は専用のテキストとなる
          action = premess + ",\n\m attempts to push\n\m away #{targetname}'s protective slime!" if target.tribe_slime?
        elsif target.is_a?(Game_Actor) #アクターが脱がされる
          action = premess + ",\n\m tries to take off #{targetname}'s clothes!"
          #スライム系は専用のテキストとなる
          action = premess + ",\n\m tries to push away #{targetname}'s protective slime!" if target.tribe_slime?
        end
        avoid = ""
  #------------------------------------------------------------------------#
      when "服を脱ぐ"
  ...
  #------------------------------------------------------------------------#
      when "インサート","シェルマッチ","アクセプト","ディルドインサート"
        if target.is_a?(Game_Enemy) #エネミーをホールドする
          ...
          when 50..100
            case $data_SDB[target.class_id].name
            when "Iｍp", "Faｍiliar ", "Goblin", "Goblin Leader ", "Unique Tycoon "
              action = premess + ",\n\m pins doｗn #{targetname}'s sｍall body froｍ above!"
            when "Caster", "Little Witch", "Liliｍ", "Slave "
              action = premess + ",\n\m pins doｗn #{targetname}'s delicate body froｍ above!"
            else
              action = premess + ",\n\m pins doｗn #{targetname}'s body froｍ above!"
            end
          else
            case $data_SDB[target.class_id].name
            when "Iｍp", "Faｍiliar ", "Goblin", "Goblin Leader ", "Unique Tycoon "
              action = premess + ",\n\m pushes doｗn #{targetname}'s sｍall body!"
            when "Caster", "Little Witch", "Liliｍ", "Slave "
              action = premess + ",\n\m pushes doｗn #{targetname}'s delicate body!"
            else
              action = premess + ",\n\m pushes doｗn #{targetname}'s body!"
            end
          end
        elsif target.is_a?(Game_Actor) #アクターがホールドされる
          case $mood.point
          when 50..100
            action = "#{targetname},\n\m has been pinned doｗn by #{myname}!"
          else
            if target == $game_actors[101]
            action = "#{myname},\n\m forcibly sits doｗn on top of #{targetname}!"
            else
            action = "#{targetname},\n\m has been pushed doｗn by #{myname}!"
            end
          end
        end
        avoid = ""
      ...
      when "エキサイトビュー"
        if target.is_a?(Game_Enemy) #エネミーをホールドする
          ...
          if myself.full_nude?
            ...
            when 50..100
              if target == $game_actors[101]
              action = "Giving #{targetname} a full vieｗ of her nethers,\n\m #{myname} opens up her crotch ｗith her fingers\n\m and starts loｗering herself over his face!"
              else
              action = "Giving #{targetname} a full vieｗ of her nethers,\n\m #{myname} opens up her crotch ｗith her fingers\n\m and starts loｗering herself over her face!"
              end
            else
              if target == $game_actors[101]
              action = "Shoｗing #{targetname} a clear vieｗ of her nethers,\n\m #{myname} tries to ｍash herself doｗn\n\m  on his face!"
              else
              action = "Shoｗing #{targetname} a clear vieｗ of her nethers,\n\m #{myname} tries to ｍash herself doｗn\n\m  on her face!"
              end
            end
            ...
            when "Caster","Faｍiliar ","Little Witch","Witch "
              action = "Standing over #{targetname}'s face,\n\m #{myname} lifts up her skirt to give him a clear vieｗ!"
            when "Lesser Succubus ","Succubus"
              action = "Ｈovering over #{targetname}'s face,\n\m#{myname}'s revealing panties can be clearly seen!"
            when "Iｍp","Devil "
              if target == $game_actors[101]
              action = "Flying over #{targetname}'s face,\n\m#{myname} gives him a vieｗ betｗeen her strong legs!"
              else
              action = "Flying over #{targetname}'s face,\n\m#{myname} gives her a vieｗ betｗeen her strong legs!"
              end
            when "Sliｍe"
              if target == $game_actors[101]
              action = "Slithering over #{targetname},\n\m #{myname} covers him in sliｍe and begins\n aligning herself\n\m  to his head!"
              else
              action = "Slithering over #{targetname},\n\m #{myname} covers her in sliｍe and begins\n aligning herself\n\m  to her head!"
              end
            when "Nightｍare"
              action = "Ｈovering over #{targetname}'s face,\n\m#{myname} offers an enticing vieｗ through her thin panties!"
            else
              if target == $game_actors[101]
              action = "#{myname} aligns herself over #{targetname}'s face,\n\m and begins to loｗer herself!"
              else
              action = "#{myname} aligns herself over #{targetname}'s face,\n\m and begins to loｗer herself!"
              end
            end
  ...
  #------------------------------------------------------------------------#
      when "ドロウネクター"
        if target.is_a?(Game_Enemy) #エネミーをホールドする
          action = premess + ",\n\m buries face-first betｗeen #{targetname}'s legs,\m\n deep into her crotch!"
        elsif target.is_a?(Game_Actor) #アクターがホールドされる
          if myself.positive?
  ...
  #------------------------------------------------------------------------#
      when "フラッタナイズ"
        if target.is_a?(Game_Enemy) #エネミーをホールドする
          action = premess + ",\n\m pulls #{targetname} in close!"
          action = premess + ",\n\m turns #{targetname} around to ｍeet face-to-face!" if target.holding?
        elsif target.is_a?(Game_Actor) #アクターがホールドされる
          if myself.positive?
            action = premess + ",\n\m pulls #{targetname} in close!"
          elsif myself.negative?
            action = premess + " closes her eyes,\n\m bringing her face close to #{targetname}'s lips!"
          else
            action = premess + ",\n\m pulls #{targetname} in close!"
          end
        end
  ...
  #------------------------------------------------------------------------#
      when "エンブレイス"
        if target.is_a?(Game_Enemy) #エネミーをホールドする
          action = premess + ",\n\m clings to #{targetname} froｍ the rear!"
          action = premess + ",\n\m bends over #{targetname}, eｍbracing her froｍ behind!" if target.holding?
        elsif target.is_a?(Game_Actor) #アクターがホールドされる
  ...
  #------------------------------------------------------------------------#
      when "ペリスコープ"
        if target.is_a?(Game_Enemy) #エネミーをホールドする
          action = premess + ",\n\m clings to #{targetname}'s ｗaist!"
        elsif target.is_a?(Game_Actor) #アクターがホールドされる
...
################################################################################
  ...
  #------------------------------------------------------------------------#
      when 9     #トーク
        case $mood.point
        when 50..100
          action = premess + ",\n\m ｗhispers ｑuietly into #{targetname}'s ear!"
        else
          action = premess + ",\n\m starts speaking to #{targetname}!"
        end
        avoid = ""
  #------------------------------------------------------------------------#
      when 81    #キッス
      ...
      when 82    #バスト
        ...
        if target.nude?
          if skill.element_set.include?(71) #手を使用
            ...
            action = premess + " uses her hands to caress\n\m #{targetname}'s #{target.bustsize}!"
            end
            action = premess + " reaches out a hand to caress\n\m #{targetname}'s #{target.bustsize}!" if myself.holding?
          elsif skill.element_set.include?(72) #口を使用
            if myself == $game_actors[101]
            action = premess + " caresses #{targetname}'s #{target.bustsize}\n\m ｗith his ｍouth!"
            else
            action = premess + " caresses #{targetname}'s\n\m #{target.bustsize} ｗith her ｍouth!"
            end
            if myself == $game_actors[101]
            action = premess + " shifts his body, suckling \n\m#{targetname}'s #{target.bustsize} ｗith his ｍouth!" if myself.holding?
            else
            action = premess + " shifts her body, suckling \n\m#{targetname}'s #{target.bustsize} ｗith her ｍouth!" if myself.holding?
            end
          end
        else
          if skill.element_set.include?(71) #手を使用
            if myself == $game_actors[101]
            action = premess + " uses his hands to caress #{targetname}'s\n\m #{target.bustsize} through her clothes!"
            else
              if target == $game_actors[101]
              action = premess + " uses her hands to caress\n\m #{targetname}'s #{target.bustsize} through his clothes!"
              else
              action = premess + " uses her hands to caress\n\m #{targetname}'s #{target.bustsize} through her clothes!"
              end
            end
            if target == $game_actors[101]
            action = premess + " reaches out a hand to caress #{targetname}'s\n\m #{target.bustsize}\n\m through his clothes!" if myself.holding?
            else
            action = premess + " reaches out a hand to caress\n\m #{targetname}'s #{target.bustsize}\n\m through her clothes!" if myself.holding?
            end
          elsif skill.element_set.include?(72) #口を使用
            if myself == $game_actors[101]
            ...
            if myself == $game_actors[101]
            action = "Shifting his body, he suckles \n\m#{targetname}'s #{target.bustsize} through her\n\m clothes ｗith his ｍouth!" if myself.holding?
            else
              if target == $game_actors[101]
              action = "Shifting her body, she suckles \n\m#{targetname}'s #{target.bustsize} through his\n\m clothes ｗith her ｍouth!" if myself.holding?
              else
              action = "Shifting her body, she suckles \n\m#{targetname}'s #{target.bustsize} through her\n\m clothes ｗith her ｍouth!" if myself.holding?
              end
            end
      ...
      when 83    #ヒップ
        ...
        if target.nude?
          if skill.element_set.include?(71) #手を使用
            ...
            action = premess + " reaches out a hand to caress\n\m #{targetname}'s ass!" if myself.holding?
          elsif skill.element_set.include?(72) #口を使用
            if myself == $game_actors[101]
            ...
            if myself == $game_actors[101]
            action = premess + " shifts his body to caress #{targetname}'s\n\m ass ｗith his ｍouth!" if myself.holding?
            else
            action = premess + " shifts her body to caress\n\m #{targetname}'s ass ｗith her ｍouth!" if myself.holding?
            end
          end
        else
          if skill.element_set.include?(71) #手を使用
            if myself == $game_actors[101]
            action = premess + " uses his hands to caress #{targetname}'s\n\m ass through her #{pantsu}!"
            else
              if target == $game_actors[101]
              action = premess + " uses her hands to caress\n\m #{targetname}'s butt through his #{pantsu}!"
              else
              action = premess + " uses her hands to caress\n\m #{targetname}'s ass through her #{pantsu}!"
              end
            end
            if target == $game_actors[101]
            action = premess + " reaches out a hand to caress\n\m #{targetname}'s butt through his #{pantsu}!" if myself.holding?
            else
            action = premess + " reaches out a hand to caress\n\m #{targetname}'s ass through her #{pantsu}!" if myself.holding?
            end
          elsif skill.element_set.include?(72) #口を使用
            if myself == $game_actors[101]
            action = premess + " uses his ｍouth to caress #{targetname}'s\n\m ass through her #{pantsu}!"
            ...
            if myself == $game_actors[101]
            action = premess + " shifts his body to caress #{targetname}'s\n\m ass through her #{pantsu}!" if myself.holding?
            else
              if target == $game_actors[101]
              action = premess + " shifts her body to caress\n\m #{targetname}'s butt through his #{pantsu}!" if myself.holding?
              else
      ...
      when 84    #クロッチ
        ...
        if target.nude?
          if skill.element_set.include?(71) #手を使用
            ...
            if myself == $game_actors[101]
            action = premess + " shifts his body to caress\n\m #{targetname}'s crotch ｗith his ｍouth!" if myself.holding?
            else
            action = premess + " shifts her body to caress\n\m #{targetname}'s crotch ｗith her ｍouth!" if myself.holding?
            end
          end
        else
          if skill.element_set.include?(71) #手を使用
            if myself == $game_actors[101]
            action = premess + " uses his hands to caress #{targetname}'s\n\m pussy through her #{pantsu}!"
            else
              if target == $game_actors[101]
              action = premess + " uses her hands to caress\n\m #{targetname}'s penis through his #{pantsu}!"
              else
              action = premess + " uses her hands to caress\n\m #{targetname}'s pussy through her #{pantsu}!"
              end
            end
            if target == $game_actors[101]
            action = premess + " reaches out a hand to caress\n\m #{targetname}'s penis through his #{pantsu}!" if myself.holding?
            else
            action = premess + " reaches out a hand to caress\n\m #{targetname}'s pussy through her #{pantsu}!" if myself.holding?
            end
          elsif skill.element_set.include?(72) #口を使用
            if myself == $game_actors[101]
            action = premess + " uses his ｍouth to caress #{targetname}'s\n\m pussy through her #{pantsu}!"
  ...
  #------------------------------------------------------------------------#
      when 55   #ライディング
        if myself.nude?
          action = "Straddling #{targetname}'s face,\n\m #{myname} shakes her hips back and forth!"
          action = "Riding on top of #{targetname}'s face,\n\m #{myname} grinds her hips back and forth!" if $mood.point > 50
        else
          case $data_SDB[myself.class_id].name
          when "Caster","Faｍiliar ","Little Witch","Witch "
            action = "Froｍ underneath her skirt,\n\m #{myname} presses her undergarｍents against\n\m #{targetname}'s ｍouth!"
          when "Lesser Succubus ","Succubus"
            action = "#{myname} drops doｗn, pressing\n\m her #{pantsu} against #{targetname}'s ｍouth!"
          when "Iｍp","Devil "
            action = "#{myname} drops doｗn, pressing\n\m her #{pantsu} against #{targetname}'s ｍouth!"
          when "Sliｍe"
            action = "Riding on top of #{targetname}'s face,\n\m #{myname} presses her pussy into\n\m #{targetname}'s ｍouth!"
          when "Nightｍare"
            action = "#{myname} drops doｗn, pressing\n\m her #{pantsu} against #{targetname}'s ｍouth!"
          else
            action = "#{myname} drops doｗn, pressing\n\m her pussy against #{targetname}'s ｍouth!"
          end
        end
  #------------------------------------------------------------------------#
      when 91   #ツーパフ
  ...
  #------------------------------------------------------------------------#
      when 71,61   #リック
        if target.full_nude?
          ...
          when 50..100
            if myself == $game_actors[101]
            action = premess + " licks #{targetname}'s\n\m pussy ｗith his tongue!"
            else
            action = premess + " licks #{targetname}'s\n\m pussy ｗith her tongue!"
            end
          else
            if myself == $game_actors[101]
            action = premess + " caresses #{targetname}'s\n\m pussy ｗith his tongue!"
          ...
          when "Sliｍe"
            if myself == $game_actors[101]
            action = "Through her thick sliｍe, #{myname}\n\m pushes against #{targetname}'s pussy ｗith his tongue!"
            else
            action = "Through her thick sliｍe, #{myname}\n\m pushes against #{targetname}'s pussy ｗith her tongue!"
            end
          when "Nightｍare"
            if myself == $game_actors[101]
  ...
  #------------------------------------------------------------------------#
      when 72   #リック(対尻)
        ...
        when 50..100
          if myself == $game_actors[101]
          action = premess + " pushes against #{targetname}'s\n\m sphincter ｗith his tongue!"
          else
          action = premess + " pushes against #{targetname}'s\n\m sphincter ｗith her tongue!"
          end
        else
          if myself == $game_actors[101]
          action = premess + " caresses #{targetname}'s\n\m sphincter ｗith his tongue!"
  ...
  #------------------------------------------------------------------------#
      when 148   #アピール
        ...
        when "Caster" #
          ...
          action = premess + " invites the eneｍy ｗith\n\m her defenseless appearance!" if myself.nude?
        when "Little Witch" #
  ...
  #------------------------------------------------------------------------#        
      when 298   #フリーアクション
        ...
        when "Nightｍare" #
          ...
          action = premess + " stares at #{targetname}'s\m\n face ｗith sleepy eyes...." if $game_variables[17] >= 50
        when "Caster" #
          action = premess + " is tidying up her clothes..." if not myself.nude?
          action = premess + ", as through having just noticed, ｑuickly starts fixing her clothes..." if not myself.nude? and $mood.point > 25
...
#          action = premess + "くすくす笑っている……"
  ...
  #------------------------------------------------------------------------#
      when 319   #手攻めペニス焦
        if target.nude?
          action = premess + " gently touches #{targetname}'s\n\m penis ｗith her hand!"
          action = premess + " gently brushes #{targetname}'s\n\m penis ｗith her finger!" if $game_variables[17] > 50
          action += "\n\m precuｍ dribbles out froｍ the pleasure!" if target.lub_male >= 60
        else
          if target == $game_actors[101]
          action = "#{myname} gently strokes #{targetname}'s\n\m penis through his #{pantsu}!"
          else
          action = "#{myname} gently strokes #{targetname}'s\n\m penis through her #{pantsu}!"
          end
        end
      when 320   #手攻めペニス
        if target.nude?
          ...
          action = premess + " strokes #{targetname}'s\n\m penis ｗith her finger!" if $game_variables[17] > 50
          action += "\n\m precuｍ is spilling out froｍ the intense pleasure!" if target.lub_male >= 60
        else
          if target == $game_actors[101]
      ...
      when 321   #手攻めペニス強
        if target.nude?
          #テキスト調整
          action = premess + "#{tec},\n\m puｍps #{targetname}'s penis ｗith her hand!"
          action = premess + "#{tec},\n runs her fingers over #{targetname}'s penis!" if $game_variables[17] > 50
          action += "\n\m precuｍ is spilling out froｍ the intense pleasure!" if target.lub_male >= 60
          action.gsub!("様子で","手つきで") #表現の変更
        else
          if target == $game_actors[101]
          action = "#{myname} ｑuickly slips her\n\m hand into #{targetname}'s #{pantsu},\n\m running her hands over his penis!"
          else
          action = "#{myname} ｑuickly slips her\n\m hand into #{targetname}'s #{pantsu},\n\m running her hands over her penis!"
          end
        end
      when 322   #手攻めペニス必殺
        if target.nude?
          action = premess + "#{tec} happily plays\n\m around ｗith #{targetname}'s penis！"
          action += "\n\m precuｍ is spilling out froｍ the intense pleasure!" if target.lub_male >= 60
          action.gsub!("様子で","手つきで") #表現の変更
        else
          if target == $game_actors[101]
      ...
      when 323   #手攻め睾丸焦
        if target.nude?
          action = premess + " gently rubs her\n\m hand over #{targetname}'s testicles!"
          action = premess + " gently runs her\n\m fingers over #{targetname}'s testicles!" if $game_variables[17] > 50
        else
          if target == $game_actors[101]
          action = "#{myname} ｑuickly slips her\n\m hand into #{targetname}'s #{pantsu},\n\m rubbing her hands over his scrotuｍ!"
          else
          action = "#{myname} ｑuickly slips her\n\m hand into #{targetname}'s #{pantsu},\n\m rubbing her hands over her scrotuｍ!"
          end
        end
      when 324   #手攻め睾丸必殺
        if target.nude?
          action = premess + " ｍassages #{targetname}'s\n\m\ testicles ｗith her hand!"
          action = premess + " feels up #{targetname}'s\n\m\ testicles ｗith her fingers!" if $game_variables[17] > 50
        else
          if target == $game_actors[101]
          action = "#{myname} ｑuickly slips her\n\m hand into #{targetname}'s #{pantsu},\n\m feeling up his scrotuｍ!"
          else
          action = "#{myname} ｑuickly slips her\n\m hand into #{targetname}'s #{pantsu},\n\m feeling up her scrotuｍ!"
          end
        end
      when 325   #手攻めペニス追撃
        if target.nude?
          action = premess + "#{tec} continues to\n\m caress #{targetname}'s penis soｍe ｍore!"
          action += "\n\m precuｍ keeps spilling out froｍ the intense pleasure!" if target.lub_male >= 60
        else
          action = "#{myname} slips her hand in again\n\m to grope #{targetname}'s penis soｍe ｍore!"
        end
      when 326   #手攻め睾丸追撃
        if target.nude?
          action = premess + "#{tec} continues to\n\m fondle #{targetname}'s balls soｍe ｍore!"
        else
          action = "#{myname} slips her hand in again\n\m to fondle #{targetname}'s balls soｍe ｍore!"
        end
  #------------------------------------------------------------------------#
      when 328   #手攻め胸焦
        if target.nude?
  ...
  #------------------------------------------------------------------------#
      when 334   #手攻めアソコ焦
        if target.nude?
          ...
          action = premess + " gently rubs #{targetname}'s crotch\m\n through her #{pantsu}!"
        end
      when 335   #手攻めアソコ
        if target.nude?
  ...
  #------------------------------------------------------------------------#
      when 375   #口攻めペニス焦
        if target.nude?
          ...
          action = "#{myname} ｑuietly licks #{targetname}'s penis ｗith\n\m the tip of her tongue!" if $game_variables[17] > 50
          action += "\n\m A pleasant, sliｍy sensation runs doｗn his penis!" if target.lub_male >= 60
        else
          action = "Through the #{pantsu}, #{myname}\n\m tenderly kisses betｗeen #{targetname}'s crotch!"
        end
      when 376   #口攻めペニス
        if target.nude?
          ...
          action = "Through the #{pantsu},\n\m #{myname} kisses betｗeen #{targetname}'s crotch!"
        end
      when 377   #口攻めペニス強
        if target.nude?
