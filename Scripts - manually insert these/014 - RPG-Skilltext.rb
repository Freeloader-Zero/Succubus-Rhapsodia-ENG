        ...
        if target.is_a?(Game_Enemy) #ƒGƒlƒ~[‚ğƒz[ƒ‹ƒh‚·‚é
          ...
          when 50..100
            ...
            when "I‚p", "Familiar", "Goblin", "Goblin Leader ", "Unique Tycoon "
              action = premess + ",\n\ pins down #{targetname}'s s‚all body fro‚ above!"
            when "Caster", "Little Witch", "Lili‚", "Slave "
              action = premess + ",\n\ pins down #{targetname}'s delicate body fro‚ above!"
            else
              action = premess + ",\n\ pins down #{targetname}'s body fro‚ above!"
            end
          else
            case $data_SDB[target.class_id].name
            when "I‚p", "Familiar", "Goblin", "Goblin Leader ", "Unique Tycoon "
              action = premess + ",\n\ pushes down #{targetname}'s s‚all body!"
            when "Caster", "Little Witch", "Lili‚", "Slave "
          ...
          when 50..100
            ...
            action = "#{myname},\n\ forcibly sits down on top of #{targetname}!"
            else
  ...
  #------------------------------------------------------------------------#
      when "ƒGƒ“ƒuƒŒƒCƒX"
        if target.is_a?(Game_Enemy) #ƒGƒlƒ~[‚ğƒz[ƒ‹ƒh‚·‚é
          ...
          action = "#{myname} bends over #{targetname},\n\ e‚bracing fro‚ behind!" if target.holding?
        end
        avoid = ""
  #------------------------------------------------------------------------#
      when "ƒyƒŠƒXƒR[ƒv"
  ...
  #------------------------------------------------------------------------#
      when 148   #ƒAƒs[ƒ‹
        ...
        when "Sli‚e" #
          action = premess + " poses suggestively!"
        when "Night‚are" #
          action = premess + " invites the ene‚y with\n\ her sleepy eyes!"
        when "Caster" #
  ...
  #------------------------------------------------------------------------#        
      when 298   #ƒtƒŠ[ƒAƒNƒVƒ‡ƒ“
        ...
        when "Werecat " #
          ...
          action = premess + " is purring......" if $game_variables[17] >= 50
        when "Goblin" #
...
#          action = premess + "‚­‚·‚­‚·Î‚Á‚Ä‚¢‚écc"
  ...
  #------------------------------------------------------------------------#
      ...
      when 321   #èU‚ßƒyƒjƒX‹­
        if target.nude?
          #ƒeƒLƒXƒg’²®
          action = premess + "#{tec},\n\ pu‚ps #{targetname}'s penis with her hand!"
      ...
      when 322   #èU‚ßƒyƒjƒX•KE
        if target.nude?
          action = premess + "#{tec} happily plays\n\ around with #{targetname}'s penisI"
          action += "\n\ precu‚ is spilling out fro‚ the intense pleasure!" if target.lub_male >= 60
          action.gsub!("—lq‚Å","è‚Â‚«‚Å") #•\Œ»‚Ì•ÏX
        else
          if target == $game_actors[101]
  ...
  #------------------------------------------------------------------------#
      when 375   #ŒûU‚ßƒyƒjƒXÅ
        if target.nude?
          action = "#{myname} gently kisses up and down #{targetname}'s penis!"
          action = "#{myname} quietly licks #{targetname}'s penis with\n\ the tip of her tongue!" if $game_variables[17] > 50
          action += "\n\ A pleasant, sli‚y sensation runs down his penis!" if target.lub_male >= 60
        else
          action = "Through the #{pantsu}, #{myname}\n\ tenderly kisses between #{targetname}'s crotch!"
        end
      when 376   #ŒûU‚ßƒyƒjƒX
        if target.nude?
          action = premess + "ã‚ÅA\n\#{targetname}‚ÌƒyƒjƒX‚ğär‚ß‚Ä‚«‚½I"
          action = "#{myname} licks #{targetname}'s penis with the tip of her tongue!" if $game_variables[17] > 50
          action += "\n\ A pleasant, sli‚y sensation runs down his penis!" if target.lub_male >= 60
        else
          action = "Through the #{pantsu},\n\ #{myname} kisses between #{targetname}'s crotch!"
        end
  ...
  #------------------------------------------------------------------------#
      when 766   #ƒ‰ƒCƒfƒBƒ“ƒOÅ
        if myself.nude?
          ...
          when "Lesser Succubus ","Succubus"
            action = premess + "drops down her hipsA\n\#{targetname}‚ÌŒû‚É#{pantsu}‚ğ‰Ÿ‚µ•t‚¯‚½I"
          when "I‚p","Devil "
            action = premess + "drops down her hipsA\n\#{targetname}‚ÌŒû‚É#{pantsu}‚ğ‰Ÿ‚µ•t‚¯‚½I"
          when "Sli‚e"
