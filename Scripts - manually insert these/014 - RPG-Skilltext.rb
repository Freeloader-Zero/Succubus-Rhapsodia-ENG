        ...
        if target.is_a?(Game_Enemy) #ƒGƒlƒ~[‚ğƒz[ƒ‹ƒh‚·‚é
          ...
          when 50..100
            ...
            when "I‚p", "Familiar", "Goblin", "Goblin Leader", "Unique Tycoon"
              action = premess + ",\n\ pins down #{targetname}'s small body fro‚ above!"
  ...
  #------------------------------------------------------------------------#
      ...
      when 321   #èU‚ßƒyƒjƒX‹­
        if target.nude?
          ...
          action += "\n\ precum is spilling out fro‚ the intense pleasure!" if target.lub_male >= 60
          action.gsub!("—lq‚Å","è‚Â‚«‚Å") #•\Œ»‚Ì•ÏX, unnecessary; can skip this
        else
          if target == $game_actors[101]
