  ...
  def make_text_aftertalk
    ...
    when "視姦"
      ...
      when 77 #視姦を最初から見なかった場合
        m = "#{master} resists teｍptation,\\n averting his eyes froｍ #{speaker}!"
      when 78 #視姦を途中で断った場合
        m = "#{master} ｍanages to peel aｗay,\\n averting his eyes froｍ #{speaker}!"
      when 79 #視姦しすぎて暴走した場合
        m = "#{master} cannot look aｗay froｍ the sight before his eyes,\\n falling to #{speaker}'s teｍptation!"
      #継続している場合
      else
      ...
      #●愛撫を受け入れた場合
        ...
        when "♀"
          ...
          if $game_actors[101].critical == true
            ...
            when 20,10
              if $msg.chain_attack == true
                m = "#{speaker} continues bouncing ｗildly\\n on #{master}'s penis!"
              else
...
#        m = "#{master}は#{action}、\\n#{speaker}の菊座に舌を這わせた！" if $game_variables[17] > 50 
    ...
    when "交合"
      ...
      when "甘え性"
        ...
        m = "#{speaker} craｗls doｗn on all fours,\\n staring at #{master} ｗith longing eyes!" if $game_variables[17] > 50 #後
      when "不思議"
        m = "#{speaker} sloｗly opens up her legs, ｗatching \\n #{master} as she lifts up her pelvis!" #前
