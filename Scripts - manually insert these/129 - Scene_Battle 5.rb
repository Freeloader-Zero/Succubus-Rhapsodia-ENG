  ...
  def make_basic_action_result
    ...
    if @active_battler.current_action.basic == 3
      ...
      if @active_battler.hidden == false and @active_battler.another_action == false
        if @active_battler.is_a?(Game_Enemy) and not $game_temp.first_attack_flag == 1
          # ★バトルログを表示
          $game_temp.battle_log_text += @active_battler.name + "は様子を見ている……\q"
        else
          @wait_count = 0
        end
        @phase4_step = 6
    ...
    if @active_battler.current_action.basic == 2
      ...
      if @active_battler.can_escape?
        ...
        # ★バトルログを表示
        $game_temp.battle_log_text += @active_battler.name + "は逃げ出した！\w\q"
        # ウェイトを再設定
        #▼システムウェイト
        case $game_system.ms_skip_mode
        when 3 #手動送りモード
    ...
    if @active_battler.current_action.basic == 5
      ...
      if @active_battler.can_escape?
        ...
        # バトルログを表示
        $game_temp.battle_log_text += actor_1.name + "は" + actor_2.name + "と\w\n入れ替わった！\w\q"
        # ステータス画面をリフレッシュ
